...
LLM 输出 Markdown 简历
  ↓
简历快照持久化：保存到 ~/.resume_agent/users/{user_id}/resumes/{resume_id}.md
  ↓
resume_renderer.py
  ↓
├── Markdown → HTML (python-markdown)
│     ↓
│   HTML + CSS 模板 → PDF (weasyprint)
│     ↓
│   返回 PDF 文件流
│
├── Markdown → DOCX (python-docx 模板)
│     ↓
│   返回 DOCX 文件流
│
└── Markdown 原文
...
- **P4 优化**：使用 MCP PDF 服务（独立进程/容器）隔离渲染，避免影响主服务

### 6.2 简历快照持久化

LLM 生成简历后，将 Markdown 内容持久化存储，与对话会话解耦：
...
存储路径: ~/.resume_agent/users/{user_id}/resumes/{resume_id}.md
  ↓
通过 SSE 推送 resume_id 给前端
  data: {"type": "resume_generated", "resume_id": "resume_20240414_abc123"}
```
...
模板存储在 `resume_agent/templates/` 目录下，为 weasyprint CSS 模板。
